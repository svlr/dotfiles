...
# Убираем стандартное поведение Ctrl+C (SIGINT)
stty intr undef
...
# Ctrl+C → копирует текущую строку в буфер X11
function copy_clipboard() {
      print -rn -- "$BUFFER" | xclip -selection clipboard
        zle reset-prompt
    }
zle -N copy_clipboard
bindkey '^C' copy_clipboard

# Ctrl+Z → отправляет SIGINT (код выхода 130)
function send_sigint() {
      kill -SIGINT $$
  }
zle -N send_sigint
bindkey '^Z' send_sigint
